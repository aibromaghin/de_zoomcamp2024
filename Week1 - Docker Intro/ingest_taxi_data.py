...
def main(params):
    ...
    for batch in df_iter:
        ...
        if '.parquet' in file_name:
            ...
            batch_df = batch
        print(f'Inserting batch number {count} into table')
        
        # Insert batch and time insert
        b_start = time()
        batch_df.to_sql(name=tb, con=engine, if_exists='append')
        b_end = time()

        print(f'... Inserted! Time to insert was: {b_end-b_start:10.3f} seconds. \n')
    
    t_end = time
    print(f'Completed. Total time taken was {t_end - t_start:10.3f} seconds for {count} batches.')

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Loading data from .parquet file url into a Postgres database.')
